...
// === БЕЗОПАСНОЕ ЧТЕНИЕ (Защита от крашей) ===
...
window.open_enchant_modal = function() {
    ...
    if (!modal) {
        modal = document.createElement('div'); modal.id = 'enchant-modal'; modal.className = 'modal-overlay'; modal.style.zIndex = '9999';
...
window.open_profile = function(user) { let p = cached_players[user]; if(!p) return; let s = p.stats || []; let c = s[24] || '#ffffff'; let is_og = s[25] || 0; let ptaps = s[38] || 0; let pwins = s[39] || 0; let ploss = s[40] || 0; let pstr = s[41] || 0; let ptitle = s[42] || ""; let wr = (pwins + ploss) > 0 ? Math.floor((pwins / (pwins + ploss)) * 100) : 0; let d_rank = parseInt(p.donate_rank) || 0; let pn = document.getElementById('prof-name'); if(pn) { pn.innerText = user; pn.style.color = c; } let pt = document.getElementById('prof-title'); if(pt) { pt.innerText = ptitle ? `[${ptitle}]` : ""; pt.style.color = "#aaa"; } let po = document.getElementById('prof-og'); if(po) po.style.display = is_og ? 'block' : 'none'; let pr = document.getElementById('prof-rank'); if(pr) pr.innerText = RANKS[p.rank || 0] || "Бронза I"; let pd = document.getElementById('prof-donate'); if(pd) { pd.innerText = RANKS_INFO[d_rank].name; pd.style.color = RANKS_INFO[d_rank].color; } let pc = document.getElementById('prof-club'); if(pc) pc.innerText = p.club || "Нет"; let pv = document.getElementById('prof-vrgk'); if(pv) pv.innerText = fmt(p.vrgk || 0); let ps = document.getElementById('prof-skr'); if(ps) ps.innerText = fmt(p.skrepki || 0); let ptap = document.getElementById('prof-taps'); if(ptap) ptap.innerText = fmt(ptaps); let pw = document.getElementById('prof-wl'); if(pw) pw.innerText = `${pwins} / ${ploss}`; let pwr = document.getElementById('prof-wr'); if(pwr) pwr.innerText = `${wr}%`; let pws = document.getElementById('prof-winstreak'); if(pws) pws.innerText = pstr; let pWrap = document.getElementById('prof-pic-wrap'); if(pWrap) { if (p.rank >= 21) { pWrap.classList.add('neon-pro'); } else { pWrap.classList.remove('neon-pro'); } } let pm = document.getElementById('profile-modal'); if(pm) pm.style.display = 'flex'; };
